...
class EditarItinerario extends StatelessWidget {
  final Function(Map<String, dynamic>) onSave;
  final Map<String, dynamic> registro;

  const EditarItinerario(
      {Key? key, required this.onSave, required this.registro})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: EditForm(
        title: 'Editar Registro de Itinerario',
        onSave: onSave,
        registro: registro,
      ),
    );
...
class EditForm extends StatefulWidget {
  ...
  const EditForm(
      {required this.onSave, required this.title, required this.registro});
...
class _EditFormState extends State<EditForm> {
  ...
  void initState() {
    super.initState();
    //Llenar los controladores con la infomración del registro al inicar el archivo

    _nombreController = TextEditingController(text: widget.registro['nombre']);
    _diaIngreso = widget.registro['dia_ingreso'];
    _horaEntrada = widget.registro['hora_entrada'];
    _horaSalida = widget.registro['hora_salida'];
    _horaComida = widget.registro['hora_comida'];
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Image.asset(
                  'images/editar.jpg', // Reemplaza con la ruta de tu imagen
                  fit: BoxFit.cover,
                ),
                const SizedBox(height: 16.0),
                // Agrega tus cajas de texto aquí
                const Text(
                  'Editar el itinerario de una mascota a detalle',
                  style: TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold),
                ),
                const SizedBox(height: 8.0),
                const Text(
                  'Por favor, edite los campos necesarios para que se pueda actualizar de forma existosa el itinerario de la mascota correspondiente',
  ...
  void _actualizarRegistro() async {
    // Usar el mapa con los datos del registro
    _registro = {
    ...
    await databaseHelper.updateRegistro(_registro);
